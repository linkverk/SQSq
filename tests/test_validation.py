...
# ── username (C1: case-insensitive) ───────────────────────────────────────
def test_username_lowercased_on_validate():
    assert v.validate_username("MyUser01") == "myuser01"


def test_username_uppercase_accepted_case_insensitive():
    assert v.is_valid_username("ABCDEFGH") is True
...
# ── A2: employee-ID is now 2..10 digits, no EMP prefix ────────────────────
@pytest.mark.parametrize("good", ["12", "1234567", "1234567890"])
def test_employee_id_accepts_numeric(good):
    assert v.validate_employee_id(good) == good


@pytest.mark.parametrize("bad", ["EMP123456", "1", "12345678901", "12a45"])
def test_employee_id_rejects_non_numeric_or_out_of_range(bad):
